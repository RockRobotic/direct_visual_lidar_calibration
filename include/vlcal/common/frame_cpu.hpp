...
struct FrameCPU : public Frame
{
  public:
     ...
     */
    template <typename T, int D> FrameCPU(const Eigen::Matrix<T, D, 1> *points, int num_points);
    ...
    template <typename T> void add_times(const T *times, int num_points);
    template <typename T> void add_times(const std::vector<T> &times) { add_times(times.data(), times.size()); }

    template <typename T, int D> void add_points(const Eigen::Matrix<T, D, 1> *points, int num_points);
    template <typename T, int D, typename Alloc>
    ...
    template <typename T, int D> void add_normals(const Eigen::Matrix<T, D, 1> *normals, int num_points);
    template <typename T, int D, typename Alloc>
    ...
    template <typename T, int D> void add_covs(const Eigen::Matrix<T, D, D> *covs, int num_points);
    ...
    template <typename T> void add_intensities(const T *intensities, int num_points);
    template <typename T> void add_intensities(const std::vector<T> &intensities)
    {
        add_intensities(intensities.data(), intensities.size());
    }

    template <typename T> void add_aux_attribute(const std::string &attrib_name, const T *values, int num_points)
    {
 ...
 * @note  This algorithm randomly samples points such that the number of sampled points of each voxel becomes (more or
 * less) the same. This algorithm avoids mixing point attributes (unlike the standard voxelgrid downsampling), and thus
 * can provide spatially well-distributed point samples with several attributes (e.g., normals and covs).
 *
 * @param frame             Input points
 * @param voxel_resolution  Voxel resolution
 * @param sampling_rate     Random sampling rate in [0, 1]
 ...
 */
template <typename Func> FrameCPU::Ptr filter(const Frame::ConstPtr &frame, const Func &pred)
{
 ...
 */
template <typename Func> FrameCPU::Ptr filter_by_index(const Frame::ConstPtr &frame, const Func &pred)
{
    std::vector<int> indices;
    indices.reserve(frame->size());
    for (int i = 0; i < frame->size(); i++)
 ...
 */
template <typename Compare> FrameCPU::Ptr sort(const Frame::ConstPtr &frame, const Compare &comp)
{
