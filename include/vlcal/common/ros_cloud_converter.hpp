...
template <typename Stamp> double to_sec(const Stamp &stamp) { return stamp.sec + stamp.nanosec / 1e9; }
...
template <typename Stamp> double to_sec(const Stamp &stamp) { return stamp.toSec(); }
...
template <typename T> Eigen::Vector4d get_vec4(const void *x, const void *y, const void *z)
{
    return Eigen::Vector4d(*reinterpret_cast<const T *>(x), *reinterpret_cast<const T *>(y),
                           *reinterpret_cast<const T *>(z), 1.0);
}

static RawPoints::Ptr extract_raw_points(const PointCloud2 &points_msg,
