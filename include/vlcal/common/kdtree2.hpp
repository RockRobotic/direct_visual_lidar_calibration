 ...
 */
template <typename Frame> struct KdTree2 : public NearestNeighborSearch
{
  public:
    using Index = nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<double, KdTree2<Frame>, double>,
                                                      KdTree2<Frame>, 3, size_t>;

    KdTree2(const std::shared_ptr<const Frame> &frame)
        : frame(frame), search_eps(-1.0), index(new Index(3, *this, nanoflann::KDTreeSingleIndexAdaptorParams(10)))
    {
