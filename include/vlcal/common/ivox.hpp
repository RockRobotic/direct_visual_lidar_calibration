 ...
 *        Bai et al., "Faster-LIO: Lightweight Tightly Coupled Lidar-Inertial Odometry Using Parallel Sparse Incremental
 * Voxels", IEEE RA-L, 2022
 * @note  Only the linear iVox is implemented
 * @note  To the compatibility with other nearest neighbor search methods, this implementation returns indices that
 * encode the voxel and point IDs. The first ```point_id_bits``` (e.g., 32) bits of a point index represent the point
 * ID, and the rest ```voxel_id_bits``` (e.g., 32) bits represent the voxel ID that contains the point.
 */
struct iVox : public NearestNeighborSearch
{
  public:
    ...
    void set_insertion_dist_thresh(const double dist)
    {
        insertion_dist_sq_thresh = dist * dist;
    } ///< Minimum distance between points in a cell
    void set_lru_cycle(const int lru_cycle) { this->lru_cycle = lru_cycle; } ///< LRU clearing cycle
    void set_lru_thresh(const int lru_thresh)
    {
        this->lru_thresh = lru_thresh;
    } ///< LRU cache threshold (larger keeps more voxels)
    void set_neighbor_voxel_mode(const int mode)
    {
        offsets = neighbor_offsets(mode);
    } ///< Neighboring voxel search mode (1/7/19/27)
  ...
  protected:
    ...
    {
        return i & ((1ul << point_id_bits) - 1);
    } ///< Extract the voxel ID from an index
    ...
    neighbor_offsets(const int neighbor_voxel_mode) const;
  ...
  protected:
    ...
    static_assert(sizeof(size_t) == 8,
                  "size_t is not 8 bytes!! point_id_bits and voxel_id_bits should be tuned accordingly!!");
    ...
    using VoxelMap =
        std::unordered_map<Eigen::Vector3i, LinearContainer::Ptr, XORVector3iHash, std::equal_to<Eigen::Vector3i>,
