...
IntegratedCT_GICPFactor_<TargetFrame, SourceFrame>::IntegratedCT_GICPFactor_(
    gtsam::Key source_t0_key, gtsam::Key source_t1_key, const std::shared_ptr<const TargetFrame> &target,
    const std::shared_ptr<const SourceFrame> &source, const std::shared_ptr<NearestNeighborSearch> &target_tree)
...
IntegratedCT_GICPFactor_<TargetFrame, SourceFrame>::linearize(const gtsam::Values &values) const
{
    this->update_poses(values);
    this->update_correspondences();

    double sum_errors = 0.0;
    std::vector<gtsam::Matrix6, Eigen::aligned_allocator<gtsam::Matrix6>> Hs_00(this->num_threads,
                                                                                gtsam::Matrix6::Zero());
    std::vector<gtsam::Matrix6, Eigen::aligned_allocator<gtsam::Matrix6>> Hs_01(this->num_threads,
                                                                                gtsam::Matrix6::Zero());
    std::vector<gtsam::Matrix6, Eigen::aligned_allocator<gtsam::Matrix6>> Hs_11(this->num_threads,
                                                                                gtsam::Matrix6::Zero());
    std::vector<gtsam::Vector6, Eigen::aligned_allocator<gtsam::Vector6>> bs_0(this->num_threads,
                                                                               gtsam::Vector6::Zero());
    std::vector<gtsam::Vector6, Eigen::aligned_allocator<gtsam::Vector6>> bs_1(this->num_threads,
                                                                               gtsam::Vector6::Zero());
...
#pragma omp parallel for reduction(+ : sum_errors) schedule(guided, 8) num_threads(this->num_threads)
    ...
    auto factor = gtsam::HessianFactor::shared_ptr(new gtsam::HessianFactor(
        this->keys_[0], this->keys_[1], Hs_00[0], Hs_01[0], -bs_0[0], Hs_11[0], -bs_1[0], sum_errors));
    return factor;
}
