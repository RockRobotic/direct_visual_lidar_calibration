...
IntegratedCT_ICPFactor_<TargetFrame, SourceFrame>::IntegratedCT_ICPFactor_(
    ...
    : gtsam::NonlinearFactor(gtsam::KeyVector{source_t0_key, source_t1_key}), num_threads(1),
      max_correspondence_distance_sq(1.0), target(target), source(source)
{
    //
...
IntegratedCT_ICPFactor_<TargetFrame, SourceFrame>::linearize(const gtsam::Values &values) const
{
    ...
    auto factor = gtsam::HessianFactor::shared_ptr(
        new gtsam::HessianFactor(keys_[0], keys_[1], H_00, H_01, -b_0, H_11, -b_1, sum_errors));
    return factor;
}
...
IntegratedCT_ICPFactor_<TargetFrame, SourceFrame>::deskewed_source_points(const gtsam::Values &values, bool local)
{
    update_poses(values);
    ...
    for (int i = 0; i < frame::size(*source); i++)
    {
        ...
        const auto &pose = source_poses[time_index];
        deskewed[i] =
            pose.matrix() * (Eigen::Vector4d() << frame::point(*source, i).template head<3>(), 1.0).finished();
    }
