...
template <typename T> double get_real(const T &x) { return x.a; }

template <> double get_real(const double &x) { return x; }

class NIDCost
{
  public:
    NIDCost(const camera::GenericCameraBase::ConstPtr &proj, const cv::Mat &normalized_image,
            const Frame::ConstPtr &points, const int bins = 16)
        : proj(proj), normalized_image(normalized_image.clone()), points(points), bins(bins)
    {
        //
    ...
    template <typename T> bool operator()(const T *T_camera_lidar_params, T *residual) const
    {
