...
{
  public:
    ReprojectionCost(const camera::GenericCameraBase::ConstPtr &proj, const Eigen::Vector3d &point_3d,
                     const Eigen::Vector2d &point_2d)
        : proj(proj), point_3d(point_3d), point_2d(point_2d)
    {
    }

    ~ReprojectionCost() {}

    template <typename T> bool operator()(const T *const T_camera_lidar_params, T *residual) const
    {
