...
template <int N> class NelderMead : public Optimizer<N>
{
  public:
    struct Params
    {
        Params()
            : init_step(0.1), alpha(1.0), gamma(2.0), rho(0.5), sigma(0.5), max_iterations(1024),
              convergence_var_thresh(1e-5)
        {
        }
    ...
    virtual Result optimize(const Function &function, const VectorN &x0) override
    {
        ...
        for (int i = 0; i < params.max_iterations; i++)
        {
            ...
            if (this->particles_callback)
            {
                ...
                std::transform(x.begin(), x.end(), particles.begin(),
                               [=](const VectorM &xi) { return xi.template tail<N>(); });
