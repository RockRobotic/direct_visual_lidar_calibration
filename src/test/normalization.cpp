...
int main(int argc, char **argv)
{
    ...
    for(rosbag::MessageInstance const m : rosbag::View(bag, rosbag::TopicQuery("/livox/points"))) {
      ...
      for (int i = 0; i < raw_points->size(); i++) {
        ...
        const Eigen::Vector4d xyzi(raw_points->points[i].x(), raw_points->points[i].y(), raw_points->points[i].z(),
    raw_points->intensities[i]); voxelmap[coord] = xyzi;
      }
    }
    ...
    cloud_buffer->add_intensity(glk::COLORMAP::TURBO, ply.intensities, 1.0 / *std::max_element(ply.intensities.begin(),
    ply.intensities.end())); viewer->update_drawable("points", cloud_buffer, guik::VertexColor()); viewer->spin();
    */
