...
ImageSpaceNearestNeighborSearch::ImageSpaceNearestNeighborSearch(const cv::Mat &image, const int max_radius)
    : image(image), x_blank(build_x_blank(image)), max_radius(max_radius)
{
}
...
size_t ImageSpaceNearestNeighborSearch::knn_search(const Eigen::Vector2d &pt, size_t k,
                                                   Eigen::Vector2i *k_neighbors) const
{
    Eigen::Vector2i center = pt.cast<int>();

    const auto comp = [](const auto &lhs, const auto &rhs) { return lhs.first < rhs.first; };
    std::priority_queue<std::pair<double, Eigen::Vector2i>, std::vector<std::pair<double, Eigen::Vector2i>>,
                        decltype(comp)>
        neighbors;
