...
struct DepthContinuousEdgeExtractionVoxel
{
  public:
    ...
    std::vector<Eigen::Vector4d> extract_edges(const std::vector<Eigen::Vector4d> &planes,
            ...
            for (int j = i + 1; j < planes.size(); j++)
            {
                ...
                if (abs_n.x() > abs_n.y() && abs_n.x() > abs_n.z())
                {
                    const Eigen::Matrix2d A =
                        (Eigen::Matrix2d() << plane_a.y(), plane_a.z(), plane_b.y(), plane_b.z()).finished();
                ...
                {
                    const Eigen::Matrix2d A =
                        (Eigen::Matrix2d() << plane_a.x(), plane_a.z(), plane_b.x(), plane_b.z()).finished();
                ...
                {
                    const Eigen::Matrix2d A =
                        (Eigen::Matrix2d() << plane_a.x(), plane_a.y(), plane_b.x(), plane_b.y()).finished();
...
DepthContinuousEdgeExtraction::DepthContinuousEdgeExtraction(const DepthContinuousEdgeExtractionParams &params)
    : params(params)
{
}
...
gtsam_ext::FrameCPU::Ptr DepthContinuousEdgeExtraction::extract(const gtsam_ext::Frame::ConstPtr &points,
                                                                std::vector<gtsam_ext::FrameCPU::Ptr> *plane_points)
{
...
#pragma critical
        {
            ...
            if (plane_points)
            {
                for (const auto &pts : voxel->plane_points)
                {
                    ...
                    std::transform(pts->begin(), pts->end(), new_points->points,
                                   [](const auto &pt) { return pt.getVector4fMap().template cast<double>(); });
