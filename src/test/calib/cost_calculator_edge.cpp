...
CostCalculatorEdge::CostCalculatorEdge(const camera::GenericCameraBase::ConstPtr &proj,
                                       const VisualLiDARData::ConstPtr &data, const EdgeCostParams &params)
    : params(params), proj(proj), max_fov(estimate_camera_fov(proj, {data->image.cols, data->image.rows})), data(data)
{
...
void CostCalculatorEdge::update_correspondences(const Eigen::Isometry3d &T_camera_lidar)
{
    ...
    for (int i = 0; i < edges->size(); i++)
    {
        ...
        if ((pt_2d.array() < Eigen::Array2d::Zero()).any() ||
            (pt_2d.array() >= Eigen::Array2d(data->image.cols, data->image.rows)).any())
        {
            continue;
        }
        ...
        auto remove_loc = std::remove_if(k_neighbors.begin(), k_neighbors.end(),
                                         [&](const Eigen::Vector2i &x)
                                         { return (pt_2d - x.cast<double>()).norm() > max_correspondence_dist; });
    ...
    max_correspondence_dist =
        std::max(params.min_max_correspondence_dist, max_correspondence_dist - params.dec_max_correspondence_dist);
