...
class OutliersViewer
{
  public:
    OutliersViewer(const std::string &data_path) : data_path(data_path)
    {
        ...
        std::sort(pick_offsets.begin(), pick_offsets.end(),
                  [](const auto &lhs, const auto &rhs) { return lhs.squaredNorm() < rhs.squaredNorm(); });
        ...
        T_lidar_camera.linear() = Eigen::Quaterniond(T_lidar_camera_values[6], T_lidar_camera_values[3],
                                                     T_lidar_camera_values[4], T_lidar_camera_values[5])
                                      .toRotationMatrix();
    ...
    read_correspondences(const std::string &data_path, const std::string &bag_name,
                         const gtsam_ext::Frame::ConstPtr &points)
    {
        cv::Mat point_indices_8uc4 = cv::imread(data_path + "/" + bag_name + "_lidar_indices.png", -1);
        cv::Mat point_indices = cv::Mat(point_indices_8uc4.rows, point_indices_8uc4.cols, CV_32SC1,
                                        reinterpret_cast<int *>(point_indices_8uc4.data));

        std::ifstream matches_ifs(data_path + "/" + bag_name + "_matches.json");
        if (!matches_ifs)
        {
            std::cerr << vlcal::console::bold_red << "error: failed to open "
                      << data_path + "/" + bag_name + "_matches.json" << vlcal::console::reset << std::endl;
            abort();
        }
        ...
        for (int i = 0; i < matches.size(); i++)
        {
            ...
            if (point_index < 0)
            {
                ...
                if (point_index < 0)
                {
                    std::cerr << vlcal::console::bold_yellow << "warning: ignore keypoint in a blank region!!"
                              << vlcal::console::reset << std::endl;
                }
                continue;
            }
