...
class ReferenceCalibration
{
  ...
  private:
    void detect_sphere(const Eigen::Vector3f &clicked_pt_)
    {
        ...
        const auto filtered = gtsam_ext::filter(dataset[selected_data_id]->points, [&](const Eigen::Vector4d &p)
                                                { return (p.head<3>() - clicked_pt).norm() < 0.2; });
        const auto coeffs = detect_sphere_ransac(filtered);

        auto viewer = guik::LightViewer::instance();
        viewer->update_drawable("selected", std::make_shared<glk::PointCloudBuffer>(filtered->points, filtered->size()),
                                guik::FlatOrange().add("point_scale", 2.0f));
        viewer->update_drawable("sphere", glk::Primitives::sphere(),
                                guik::FlatColor(1.0f, 0.0f, 0.0f, 0.5f)
                                    .translate(coeffs.head<3>().cast<float>())
                                    .scale(coeffs.w())
                                    .make_transparent());
