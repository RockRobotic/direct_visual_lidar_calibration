...
class PreprocessMap
{
  ...
  ;
        ...
        if (vm.count("help") || !vm.count("map_path") || !vm.count("image_path") || !vm.count("dst_path") ||
            !vm.count("camera_model") || !vm.count("camera_intrinsics") || !vm.count("camera_distortion_coeffs"))
        {
        ...
        // Load map points
        auto lidar_points =
            load_lidar_points(map_path, vm["voxel_resolution"].as<double>(), vm["min_distance"].as<double>());
    ...
    vlcal::Frame::ConstPtr load_lidar_points(const std::string &path, double voxel_resolution, double min_distance)
    {
        ...
        std::sort(indices.begin(), indices.end(),
                  [&](const int lhs, const int rhs) { return frame->intensities[lhs] < frame->intensities[rhs]; });
    ...
    bool save_lidar_data(const std::string &dst_path, const Frame::ConstPtr &lidar_points)
    {
        ...
        if (lidar_fov < 150.0 * M_PI / 180.0)
        {
            ...
            T_lidar_camera.linear() = (Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitY()) *
        ...
        else
        {
            ...
            lidar_camera_intrinsics = {static_cast<double>(lidar_image_size[0]),
                                       static_cast<double>(lidar_image_size[1])};
        }
        ...
        auto [intensities, indices] =
            vlcal::generate_lidar_image(lidar_proj, lidar_image_size, T_lidar_camera.inverse(), lidar_points);
