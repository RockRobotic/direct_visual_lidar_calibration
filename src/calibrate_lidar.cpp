...
{
  public:
    VisualLiDARCalibration(const std::string &data_path, const boost::program_options::variables_map &vm)
        : data_path(data_path)
    {
    ...
    void calibrate(const boost::program_options::variables_map &vm)
    {
        ...
        init_T_lidar_camera.linear() =
            Eigen::Quaterniond(init_values[6], init_values[3], init_values[4], init_values[5])
                .normalized()
                .toRotationMatrix();
        ...
        {
            std::cerr << vlcal::console::bold_yellow << "warning: unknown registration type " << registration_type
                      << vlcal::console::reset << std::endl;
        }
        ...
        const std::vector<double> T_lidar_camera_values = {trans.x(), trans.y(), trans.z(), quat.x(),
