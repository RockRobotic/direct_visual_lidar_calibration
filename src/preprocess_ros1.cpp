...
{
  public:
    PointCloudReaderROS1(const std::string &bag_filename, const std::string &points_topic,
...
class PreprocessROS1 : public Preprocess
{
  protected:
    ...
    get_topics_and_types(const std::string &bag_filename) override
    {
        rosbag::Bag bag(bag_filename);
    ...
    virtual std::vector<std::string> get_point_fields(const std::string &bag_filename,
                                                      const std::string &points_topic) override
    {
        ...
        std::transform(msg->fields.begin(), msg->fields.end(), fields.begin(),
    ...
    virtual cv::Size get_image_size(const std::string &bag_filename, const std::string &image_topic) override
    {
        ...
        std::cerr << console::bold_red << "error: failed to retrieve image messages from the bag file" << console::reset
                  << std::endl;
        return cv::Size(0, 0);
    }

    virtual std::tuple<std::string, std::vector<double>, std::vector<double>>
    get_camera_info(const std::string &bag_filename, const std::string &camera_info_topic) override
    {
    ...
    virtual std::shared_ptr<PointCloudReader> get_point_cloud_reader(const std::string &bag_filename,
                                                                     ...
                                                                     const std::string &intensity_channel) override
    {
