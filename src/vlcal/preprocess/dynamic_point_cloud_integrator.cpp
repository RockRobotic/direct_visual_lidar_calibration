...
DynamicPointCloudIntegrator::DynamicPointCloudIntegrator(const DynamicPointCloudIntegratorParams &params)
    : params(params)
{
...
void DynamicPointCloudIntegrator::insert_points(const Frame::ConstPtr &raw_points_)
{
    ...
    auto points =
        randomgrid_sampling(raw_points, 0.5, static_cast<double>(params.target_num_points) / raw_points->size(), mt);
...
#pragma omp parallel for num_threads(params.num_threads)
    ...
    const gtsam::Vector6 pred_v_odom_lidar =
        gtsam::Pose3::Logmap(last_T_odom_lidar_begin.between(last_T_odom_lidar_end)) / scan_duration;
    const gtsam::Pose3 pred_T_odom_lidar_begin = last_T_odom_lidar_end;
    const gtsam::Pose3 pred_T_odom_lidar_end =
        last_T_odom_lidar_end * gtsam::Pose3::Expmap(0.75 * pred_v_odom_lidar * scan_duration);
    ...
    graph.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(0, gtsam::Pose3(pred_T_odom_lidar_begin),
                                                           gtsam::noiseModel::Isotropic::Precision(6, 1e3));
    graph.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(0, 1, gtsam::Pose3(),
                                                             gtsam::noiseModel::Isotropic::Precision(6, 1e5));
    ...
    if (params.visualize)
    {
        ...
        viewer->update_drawable("coord", glk::Primitives::coordinate_system(),
                                guik::VertexColor(last_T_odom_lidar_end.matrix()));
        viewer->update_drawable("points", std::make_shared<glk::PointCloudBuffer>(deskewed->points, deskewed->size()),
                                guik::FlatOrange());
        viewer->update_drawable("target", std::make_shared<glk::PointCloudBuffer>(target_ivox->voxel_points()),
                                guik::Rainbow());
        viewer->spin_once();
    }
}
...
void DynamicPointCloudIntegrator::voxelgrid_task()
{
    while (true)
    {
        ...
        cloud_buffer->add_intensity(glk::COLORMAP::TURBO, intensities, 1.0f / (*std::max_element(intensities.begin(),
        intensities.end()))); viewer->update_drawable("points", cloud_buffer, guik::VertexColor()); viewer->spin_once();
        */
    }
}
