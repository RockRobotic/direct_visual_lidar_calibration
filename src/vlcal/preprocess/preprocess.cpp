...
bool Preprocess::run(int argc, char **argv)
{
  ...
  ;
    ...
    if (vm.count("bag_id"))
    {
        ...
        std::cerr << vlcal::console::bold_yellow << "use only " << bag_filenames[bag_id] << vlcal::console::reset
                  << std::endl;
    ...
    if (vm.count("first_n_bags"))
    {
        ...
        std::cerr << vlcal::console::bold_yellow << "use only the following rosbags:" << vlcal::console::reset
                  << std::endl;
        for (const auto &bag_filename : bag_filenames)
        {
    ...
    // camera params
    auto [camera_model, image_size, intrinsics, distortion_coeffs] =
        get_camera_params(vm, bag_filenames.front(), camera_info_topic, image_topic);
    std::cout << "camera_model: " << camera_model << std::endl;
    std::cout << "image_size  : " << image_size.width << " " << image_size.height << std::endl;
    std::cout << "intrinsics  : " << Eigen::Map<const Eigen::VectorXd>(intrinsics.data(), intrinsics.size()).transpose()
              << std::endl;
    std::cout << "dist_coeffs : "
              << Eigen::Map<const Eigen::VectorXd>(distortion_coeffs.data(), distortion_coeffs.size()).transpose()
              << std::endl;
    ...
    for (int i = 0; i < bag_filenames.size(); i++)
    {
        ...
        auto [image, points] =
            get_image_and_points(vm, bag_filename, image_topic, points_topic, intensity_channel, num_threads_per_bag);
        ...
        // Eigen::Isometry3d T_lidar_camera2  = Eigen::Isometry3d::Identity();
        // T_lidar_camera2.linear() = (Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(-M_PI_2,
        // Eigen::Vector3d::UnitZ())).toRotationMatrix(); std::cout << points->points[0] << std::endl; std::cout <<
        // T_lidar_camera2.matrix() << std::endl; std::cout << T_lidar_camera2 * points->points[0] << std::endl;
        // std::cout << (T_lidar_camera2 * points->points[0]).cast<float>().head<3>() << std::endl;
        // for (int i = 0; i < points->size(); i++) {
        //   ply.vertices[i] = ((T_lidar_camera2 * points->points[i])).cast<float>().head<3>();
        //   }
        std::transform(points->points, points->points + points->size(), ply.vertices.begin(),
                       [](const Eigen::Vector4d &p) { return p.cast<float>().head<3>(); });
    ...
    {
        lidar_image_size = {1024 * 3, 1024 * 2};
        // T_lidar_camera.linear() = (Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(-M_PI_2,
    ...
    std::cout << "modeL: " << lidar_camera_model << std::endl;
    std::cout << "int: " << std::endl;
    for (double x : lidar_camera_intrinsics)
    {
        std::cout << x << std::endl;
    }
...
#pragma omp parallel for
    for (int i = 0; i < bag_filenames.size(); i++)
    {
        ...
        auto [intensities, indices] =
            vlcal::generate_lidar_image(lidar_proj, lidar_image_size, T_lidar_camera.inverse(), lidar_points[i]);
    ...
    std::transform(bag_filenames.begin(), bag_filenames.end(), bag_names.begin(),
                   [](const auto &path) { return std::filesystem::path(path).filename(); });
...
Preprocess::get_topics(const boost::program_options::variables_map &vm, const std::string &bag_filename)
{
    ...
    if (vm.count("auto_topic"))
    {
        ...
        for (const auto &[topic, type] : topics_and_types)
        {
            ...
            if (type.find("CameraInfo") != std::string::npos)
            {
                ...
                {
                    std::cerr << vlcal::console::bold_yellow << "warning: bag constains multiple camera_info topics!!"
            ...
            else if (type.find("Image") != std::string::npos)
            {
                ...
                {
                    std::cerr << vlcal::console::bold_yellow << "warning: bag constains multiple image topics!!"
                              << vlcal::console::reset << std::endl;
                }
                image_topic = topic;
            }
            else if (type.find("PointCloud2") != std::string::npos)
            {
                ...
                {
                    std::cerr << vlcal::console::bold_yellow << "warning: bag constains multiple points topics!!"
                              << vlcal::console::reset << std::endl;
                }
                points_topic = topic;
            }
    ...
    if (camera_info_topic.empty())
    {
        std::cerr << console::bold_yellow << "warning: failed to get camera_info topic!!" << console::reset
                  << std::endl;
    }
    if (image_topic.empty())
    {
...
std::string Preprocess::get_intensity_channel(const boost::program_options::variables_map &vm,
                                              const std::string &bag_filename, const std::string &points_topic)
{
    ...
    if (intensity_channel == "auto")
    {
        std::cerr << vlcal::console::bold_red << "error: failed to determine point intensity channel automatically"
                  << vlcal::console::reset << std::endl;
        std::cerr << vlcal::console::bold_red << "     : you must specify the intensity channel to be used manually"
                  << vlcal::console::reset << std::endl;
    }
...
Preprocess::get_camera_params(const boost::program_options::variables_map &vm, const std::string &bag_filename,
                              const std::string &camera_info_topic, const std::string &image_topic)
{
    //
    cv::Size image_size = get_image_size(bag_filename, image_topic);
    if (image_size.width == 0 && image_size.height == 0)
    {
        std::cerr << vlcal::console::bold_yellow << "warning: image size is not set (image_topic=" << image_topic << ")"
                  << vlcal::console::reset << std::endl;
    }
    ...
    {
        const std::unordered_set<std::string> valid_camera_models = {"plumb_bob", "fisheye", "equidistant", "omnidir",
                                                                     "equirectangular"};
        if (!valid_camera_models.count(camera_model))
        {
            std::cerr << vlcal::console::bold_red << "error: invalid camera model " << camera_model
                      << vlcal::console::reset << std::endl;
            ...
            std::cerr << vlcal::console::bold_red << "     : supported camera models are" << sst.str()
                      << vlcal::console::reset << std::endl;
        ...
        else
        {
            ...
            {
                std::cerr << vlcal::console::bold_red << "error: camera_intrinsics has not been set!!"
                          << vlcal::console::reset << std::endl;
            }
            if (!vm.count("camera_distortion_coeffs"))
            {
                std::cerr << vlcal::console::bold_red << "error: camera_distortion_coeffs has not been set!!"
                          << vlcal::console::reset << std::endl;
            }
            ...
            std::transform(intrinsic_tokens.begin(), intrinsic_tokens.end(), intrinsics.begin(),
                           [](const auto &token) { return std::stod(token); });
            std::transform(distortion_tokens.begin(), distortion_tokens.end(), distortion_coeffs.begin(),
                           [](const auto &token) { return std::stod(token); });
        }
...
std::pair<cv::Mat, Frame::ConstPtr>
Preprocess::get_image_and_points(const boost::program_options::variables_map &vm, const std::string &bag_filename,
                                 const std::string &image_topic, const std::string &points_topic,
                                 const std::string &intensity_channel, const int num_threads)
{
    //
    cv::Mat image = get_image(bag_filename, image_topic);
    if (!image.data)
    {
        std::cerr << vlcal::console::bold_red << "error: failed to obtain an image (image_topic=" << image_topic << ")"
                  << vlcal::console::reset << std::endl;
        abort();
    }
    ...
    while (true)
    {
        ...
        {
            std::cerr << vlcal::console::yellow << "warning: skip frame with an invalid timestamp!!"
                      << vlcal::console::reset << std::endl;
            continue;
        }
    ...
    std::sort(indices.begin(), indices.end(),
              [&](const int lhs, const int rhs) { return points->intensities[lhs] < points->intensities[rhs]; });
