...
NIDCostParams::NIDCostParams() { bins = 16; }

NIDCostParams::~NIDCostParams() {}

CostCalculatorNID::CostCalculatorNID(const camera::GenericCameraBase::ConstPtr &proj,
                                     const VisualLiDARData::ConstPtr &data, const NIDCostParams &params)
    : params(params), proj(proj), data(data), max_fov(estimate_camera_fov(proj, {data->image.cols, data->image.rows}))
{
}
