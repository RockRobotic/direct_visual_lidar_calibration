...
iVox::iVox(const double voxel_resolution, const double insertion_dist_thresh, const int lru_thresh)
    : voxel_resolution(voxel_resolution), insertion_dist_sq_thresh(insertion_dist_thresh * insertion_dist_thresh),
      lru_cycle(10), lru_thresh(lru_thresh)
{
    lru_count = 0;
...
iVox::neighbor_offsets(const int neighbor_voxel_mode) const
{
    ...
    case 7:
        return std::vector<Eigen::Vector3i, Eigen::aligned_allocator<Eigen::Vector3i>>{
            Eigen::Vector3i(0, 0, 0),  Eigen::Vector3i(1, 0, 0), Eigen::Vector3i(-1, 0, 0), Eigen::Vector3i(0, 1, 0),
...
void iVox::insert(const Frame &frame)
{
    ...
    if (voxelmap.size() >= (1ul << voxel_id_bits) - 1)
    {
        ...
        std::vector<std::pair<Eigen::Vector3i, LinearContainer::Ptr>,
                    Eigen::aligned_allocator<std::pair<Eigen::Vector3i, LinearContainer::Ptr>>>
            voxels(voxelmap.begin(), voxelmap.end());
        std::sort(voxels.begin(), voxels.end(),
                  [](const std::pair<Eigen::Vector3i, LinearContainer::Ptr> &lhs,
                     const std::pair<Eigen::Vector3i, LinearContainer::Ptr> &rhs)
...
const Eigen::Vector4d &iVox::point(const size_t i) const { return voxels[voxel_id(i)]->points[point_id(i)]; }

const Eigen::Vector4d &iVox::normal(const size_t i) const { return voxels[voxel_id(i)]->normals[point_id(i)]; }

const Eigen::Matrix4d &iVox::cov(const size_t i) const { return voxels[voxel_id(i)]->covs[point_id(i)]; }

double iVox::intensity(const size_t i) const { return voxels[voxel_id(i)]->intensities[point_id(i)]; }
