...
Eigen::Vector3d estimate_direction(const camera::GenericCameraBase::ConstPtr &proj, const Eigen::Vector2d &pt_2d)
{
    ...
    {
        return Eigen::AngleAxisd(x[0], Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(x[1], Eigen::Vector3d::UnitY()) *
...
{
    const std::vector<Eigen::Vector2d> target_corners = {
        Eigen::Vector2d(0.0, 0.0), Eigen::Vector2d(image_size[0] / 2, 0.0), Eigen::Vector2d(0.0, image_size[1] / 2)};
...
double estimate_lidar_fov(const Frame::ConstPtr &points)
{
    ...
    std::transform(points->points, points->points + points->size(), cloud->begin(),
    ...
    const auto remove_loc = std::remove_if(filtered->begin(), filtered->end(),
                                           [](const pcl::PointXYZ &pt) { return pt.getVector3fMap().norm() < 1.0; });
