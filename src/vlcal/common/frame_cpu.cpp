...
template <typename T, int D> FrameCPU::FrameCPU(const Eigen::Matrix<T, D, 1> *points, int num_points)
{
...
// add_times
template <typename T> void FrameCPU::add_times(const T *times, int num_points)
{
...
// add_points
template <typename T, int D> void FrameCPU::add_points(const Eigen::Matrix<T, D, 1> *points, int num_points)
{
...
// add_normals
template <typename T, int D> void FrameCPU::add_normals(const Eigen::Matrix<T, D, 1> *normals, int num_points)
{
    assert(num_points == size());
...
// add_covs
template <typename T, int D> void FrameCPU::add_covs(const Eigen::Matrix<T, D, D> *covs, int num_points)
{
    assert(num_points == size());
...
// add_intensities
template <typename T> void FrameCPU::add_intensities(const T *intensities, int num_points)
{
    assert(num_points == size());
...
FrameCPU::Ptr FrameCPU::load(const std::string &path)
{
    ...
    else if (boost::filesystem::exists(path + "/points_compact.bin"))
    {
        ...
        std::transform(points_f.begin(), points_f.end(), frame->points,
                       [](const Eigen::Vector3f &p) { return Eigen::Vector4d(p[0], p[1], p[2], 1.0); });
        ...
        if (boost::filesystem::exists(path + "/normals_compact.bin"))
        {
            ...
            std::transform(normals_f.begin(), normals_f.end(), frame->normals,
                           [](const Eigen::Vector3f &p) { return Eigen::Vector4d(p[0], p[1], p[2], 0.0); });
        }
        ...
        if (boost::filesystem::exists(path + "/covs_compact.bin"))
        {
            frame->covs_storage.resize(frame->size());
            frame->covs = frame->covs_storage.data();
            std::vector<Eigen::Matrix<float, 6, 1>, Eigen::aligned_allocator<Eigen::Matrix<float, 6, 1>>> covs_f(
                frame->size());
    ...
    {
        std::smatch matched;
        auto infile = itr->path().string();
        if (!std::regex_search(infile, matched, aux_name_regex))
        {
            continue;
        }
        const std::string name = matched.str(1);

        std::ifstream ifs(infile, std::ios::ate | std::ios::binary);
        ...
        {
            std::cerr << "warning: elem_size=" << elem_size << " num_points=" << frame->size() << " bytes=" << bytes
                      << std::endl;
...
FrameCPU::Ptr random_sampling(const Frame::ConstPtr &frame, const double sampling_rate, std::mt19937 &mt)
{
    ...
    std::sample(boost::counting_iterator<int>(0), boost::counting_iterator<int>(frame->size()), sample_indices.begin(),
                num_samples, mt);
...
FrameCPU::Ptr voxelgrid_sampling(const Frame::ConstPtr &frame, const double voxel_resolution)
{
    ...
    using VoxelMap = std::unordered_map<Eigen::Vector3i, Indices, Vector3iHash, std::equal_to<Eigen::Vector3i>,
    ...
    std::transform(voxelmap.begin(), voxelmap.end(), voxels.begin(),
                   [](const std::pair<Eigen::Vector3i, Indices> &x) { return x.second; });
    ...
    std::transform(voxels.begin(), voxels.end(), downsampled->points,
    ...
    if (frame->times)
    {
        ...
        std::transform(voxels.begin(), voxels.end(), downsampled->times,
                       [&](const Indices &indices)
                       {
                           double sum = 0.0;
                           for (const auto i : *indices)
    ...
    if (frame->normals)
    {
        ...
        std::transform(voxels.begin(), voxels.end(), downsampled->normals,
                       [&](const Indices &indices)
                       {
                           Eigen::Vector4d sum = Eigen::Vector4d::Zero();
                           for (const auto i : *indices)
    ...
    if (frame->covs)
    {
        ...
        std::transform(voxels.begin(), voxels.end(), downsampled->covs,
                       [&](const Indices &indices)
                       {
    ...
    if (frame->intensities)
    {
        ...
        std::transform(voxels.begin(), voxels.end(), downsampled->intensities,
                       [&](const Indices &indices)
                       {
                           double sum = 0.0;
                           for (const auto i : *indices)
...
// randomgrid_sampling
FrameCPU::Ptr randomgrid_sampling(const Frame::ConstPtr &frame, const double voxel_resolution,
                                  const double sampling_rate, std::mt19937 &mt)
{
    ...
    for (const auto &voxel : voxelmap)
    {
        ...
        {
            std::sample(voxel_indices.begin(), voxel_indices.end(), std::back_insert_iterator(indices),
                        points_per_voxel, mt);
        }
    }
...
// transform
template <>
FrameCPU::Ptr transform(const Frame::ConstPtr &frame,
...
template <> void transform_inplace(Frame::Ptr &frame, const Eigen::Transform<float, 3, Eigen::Isometry> &transformation)
{
    transform_inplace<float, Eigen::Isometry>(frame, transformation);
}

template <> void transform_inplace(Frame::Ptr &frame, const Eigen::Transform<double, 3, Eigen::Affine> &transformation)
{
    for (int i = 0; i < frame->size(); i++)
    {
        frame->points[i] = transformation * frame->points[i];
...
template <> void transform_inplace(Frame::Ptr &frame, const Eigen::Transform<float, 3, Eigen::Affine> &transformation)
{
    return transform_inplace<double, Eigen::Affine>(frame, transformation.cast<double>());
}

// statistical outlier removal
std::vector<int> find_inlier_points(const Frame::ConstPtr &frame, const std::vector<int> &neighbors, const int k,
