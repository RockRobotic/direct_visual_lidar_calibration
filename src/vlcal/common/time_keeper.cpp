...
bool TimeKeeper::validate_imu_stamp(const double imu_stamp)
{
    ...
    else if (imu_stamp < last_imu_stamp)
    {
        ...
        std::cerr << console::yellow
                  << boost::format("       : current:%.6f last:%.6f diff:%.6f") % imu_stamp % last_imu_stamp % imu_diff
                  << console::reset << std::endl;
        return false;
    }
    else if (imu_stamp - last_imu_stamp > 0.1)
    {
        std::cerr << console::yellow << "warning: large time gap between consecutive IMU data!!" << console::reset
                  << std::endl;
        std::cerr << console::yellow
                  << boost::format("       : current:%.6f last:%.6f diff:%.6f") % imu_stamp % last_imu_stamp % imu_diff
                  << console::reset << std::endl;
    ...
    {
        std::cerr << console::yellow << "warning: large time difference between points and imu!!" << console::reset
                  << std::endl;
        std::cerr << console::yellow
                  << boost::format("       : points:%.6f imu:%.6f diff:%.6f") % last_points_stamp % imu_stamp %
                         points_diff
                  << console::reset << std::endl;
    }
...
bool TimeKeeper::process(const vlcal::RawPoints::Ptr &points)
{
    ...
    else if (time_diff < 0.0)
    {
        ...
        std::cerr << console::yellow
                  << boost::format("       : current:%.6f last:%.6f diff:%.6f") % points->stamp % last_points_stamp %
                         time_diff
                  << console::reset << std::endl;
        return false;
    }
    ...
    {
        std::cerr << console::yellow << "warning: large time gap between consecutive LiDAR frames!!" << console::reset
                  << std::endl;
        std::cerr << console::yellow
                  << boost::format("       : current:%.6f last:%.6f diff:%.6f") % points->stamp % last_points_stamp %
                         time_diff
...
void TimeKeeper::replace_points_stamp(const vlcal::RawPoints::Ptr &points)
{
    ...
    if (points->times.empty())
    {
        if (first_warning)
        {
            std::cerr << console::yellow << boost::format("warning: per-point timestamps are not given!!")
                      << console::reset << std::endl;
            std::cerr << console::yellow
                      << boost::format("       : use pseudo per-point timestamps based on the order of points")
                      << console::reset << std::endl;
            first_warning = false;
        }
    ...
    {
        std::cerr << console::yellow << "warning: # of timestamps and # of points mismatch!!" << console::reset
                  << std::endl;
        points->times.resize(points->size(), 0.0);
        return;
    }
    ...
    {
        std::cerr << console::yellow
                  << boost::format("warning: negative per-point timestamp (%.6f or %.6f) found!!") %
    ...
    {
        std::cerr << console::yellow
                  << boost::format("warning: large point timestamp (%.6f > 1.0) found!!") % points->times.back()
                  << console::reset << std::endl;
        std::cerr << console::yellow
                  << boost::format("       : assume that point times are absolute and convert them to relative")
                  << console::reset << std::endl;
        std::cerr << console::yellow
                  << boost::format("       : replace_frame_stamp=%d wrt_first_frame_timestamp=%d") %
                         abs_params.replace_frame_timestamp % abs_params.wrt_first_frame_timestamp
                  << console::reset << std::endl;
    }
    ...
    if (abs_params.replace_frame_timestamp)
    {
        if (!abs_params.wrt_first_frame_timestamp || std::abs(points->stamp - points->times.front()) < 1.0)
        {
            if (first_warning)
            {
                std::cerr << console::yellow << boost::format("warning: use first point timestamp as frame timestamp")
                          << console::reset << std::endl;
                std::cerr << console::yellow
                          << boost::format("       : frame=%.6f point=%.6f") % points->stamp % points->times.front()
                          << console::reset << std::endl;
            }
        ...
        else
        {
            ...
            {
                std::cerr << console::yellow
                          << boost::format("warning: point timestamp is too apart from frame timestamp!!")
                          << console::reset << std::endl;
                std::cerr << console::yellow
                          << boost::format("       : use time offset w.r.t. the first frame timestamp")
                          << console::reset << std::endl;
                std::cerr << console::yellow
                          << boost::format("       : frame=%.6f point=%.6f diff=%.6f") % points->stamp %
                                 points->times.front() % (points->stamp - points->times.front())
                          << console::reset << std::endl;
