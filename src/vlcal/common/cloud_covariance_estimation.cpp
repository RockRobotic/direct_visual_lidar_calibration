...
CloudCovarianceEstimation::CloudCovarianceEstimation(const int num_threads)
    : regularization_method(RegularizationMethod::PLANE), num_threads(num_threads)
{
}
...
void CloudCovarianceEstimation::estimate(
    ...
    const std::vector<int> &neighbors, std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>> &normals,
...
void CloudCovarianceEstimation::estimate(
    const std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>> &points,
    const std::vector<int> &neighbors, const int k_neighbors,
...
std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>> CloudCovarianceEstimation::estimate(
    const std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>> &points,
    const std::vector<int> &neighbors, const int k_neighbors) const
{
    //
    const int k_correspondences = neighbors.size() / points.size();
    assert(k_correspondences * points.size() == neighbors.size());
...
Eigen::Matrix4d CloudCovarianceEstimation::regularize(const Eigen::Matrix4d &cov, Eigen::Vector3d *eigenvalues,
