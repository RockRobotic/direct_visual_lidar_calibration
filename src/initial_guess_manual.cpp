...
class InitialGuessManual
{
  public:
    InitialGuessManual(const std::string &data_path) : data_path(data_path), camera_image_rotate_code(-1)
    {
        ...
        std::cout << "camera_fov:" << estimate_camera_fov(proj, {image_size.width, image_size.height}) * 180.0 / M_PI
                  << "[deg]" << std::endl;
    ...
    void spin()
    {
        ...
        {
            init_T_lidar_camera.linear() = (Eigen::AngleAxisd(M_PI_2, Eigen::Vector3d::UnitY()) *
        ...
        viewer->register_ui_callback(
            ...
            [&]
            {
                ...
                vis->set_T_camera_lidar(
                    Eigen::Isometry3d(T_lidar_camera_gizmo.model_matrix().cast<double>().inverse()));
                ...
                if (ImGui::Button("Save"))
                {
                    ...
                    const std::vector<double> values = {trans.x(), trans.y(), trans.z(), quat.x(),
    ...
    void on_mouse(int event, int x, int y, int flags)
    {
        ...
        cv::line(canvas, {x - cross_size, y - cross_size}, {x + cross_size, y + cross_size}, cv::Scalar(64, 64, 64), 4,
                 cv::LINE_AA);
        cv::line(canvas, {x + cross_size, y - cross_size}, {x - cross_size, y + cross_size}, cv::Scalar(64, 64, 64), 4,
                 cv::LINE_AA);
        cv::line(canvas, {x - cross_size, y - cross_size}, {x + cross_size, y + cross_size}, cv::Scalar(0, 255, 0), 2,
                 cv::LINE_AA);
        cv::line(canvas, {x + cross_size, y - cross_size}, {x - cross_size, y + cross_size}, cv::Scalar(0, 255, 0), 2,
                 cv::LINE_AA);
        cv::imshow("image", canvas);
